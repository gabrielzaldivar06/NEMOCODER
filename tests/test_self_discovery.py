...
from nemo_coding_platform.core.self_discovery import ensure_self_mod_permissions_file, find_nemocode_repo, is_nemocode_repo


class SelfDiscoveryTests(unittest.TestCase):
    def test_find_nemocode_repo_from_nested_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            found = find_nemocode_repo(root / "src" / "nemo_coding_platform" / "core")

        self.assertEqual(found, root.resolve())

    def test_is_nemocode_repo_requires_package_tests_and_pyproject(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.assertFalse(is_nemocode_repo(root))
            (root / "src" / "nemo_coding_platform").mkdir(parents=True)
            (root / "tests").mkdir()
            (root / "pyproject.toml").write_text("name = 'nemo_coding_platform'", encoding="utf-8")

            self.assertTrue(is_nemocode_repo(root))
