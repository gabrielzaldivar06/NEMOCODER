# Getting Started with NEMOCODE

This guide walks you through pointing NEMOCODE at an existing git repository and running your first autonomous coding handoff.
...
| LM Studio | latest | [lmstudio.ai](https://lmstudio.ai) — for real Aider runs |
...
## 2. Configure LM Studio (for real Aider runs)
...
For smoke-test runs without LM Studio, leave provider mode as `fake` — the agent will write placeholder diffs.
...
2. Fill in:
   ...
   - **Provider mode**: `fake` for a smoke run; `subprocess` for real Aider with LM Studio.
...
| Aider run hangs | Verify LM Studio server is started and the model is loaded |
