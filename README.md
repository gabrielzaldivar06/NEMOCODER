...
Initial implementation scaffold for NEMO CODE: a desktop-first, highly autonomous coding platform with a local CLI harness.
...
The active embedded mutation runtime currently lives in [product/nemo_code_runtime](product/nemo_code_runtime). The CLI in `src/` is not the final product surface; it is the development harness for backend contracts that inform the NEMO CODE desktop app.

The desktop app uses the NEMO CODE runtime stack with:
...
The launcher checks the NEMO MCP SSE endpoint at `http://127.0.0.1:8765/mcp/sse`, starts the Mission Control API on `http://127.0.0.1:8787`, starts the Vite UI on `http://127.0.0.1:5173`, and writes logs under `.nemo-runtimes\logs`. The NEMO server used by this workspace exposes MCP through `/mcp/sse`; a `/health` route is not required for the MVP launcher.
