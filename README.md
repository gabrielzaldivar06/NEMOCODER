...
Initial implementation scaffold for Spacecode: a desktop-first, highly autonomous coding platform with a local CLI harness and external NEMO MCP memory integration.
...
The active embedded mutation runtime currently lives in [product/nemo_code_runtime](product/nemo_code_runtime). That path is legacy naming during the Spacecode rebrand. The CLI in `src/` is not the final product surface; it is the development harness for backend contracts that inform the Spacecode desktop app.

The desktop app uses the Spacecode runtime stack with:
