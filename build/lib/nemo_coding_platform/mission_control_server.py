...
def _provider_mode(payload: dict[str, object]) -> str:
    ...
    if provider not in {"fake", "subprocess"}:
        raise _bad_request("provider must be fake or subprocess", error_code="invalid_provider")
    return provider
...
def api_agent_message(config: MissionControlServerConfig, payload: dict[str, object]) -> dict[str, object]:
    ...
    if provider == "subprocess":
        ...
        try:
            ...
            response = (
                "[real-mode fallback] LM Studio failed, but Mission Control kept operating with real action planning. "
                "You can run the provided actions now while model connectivity is fixed."
            )
    else:
